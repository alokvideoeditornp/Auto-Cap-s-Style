        ...
        try {
          ...
          for (const file of files) {
            if (/\.(ttf|otf|ttc)$/i.test(file)) {
              let rawName = path.basename(file, path.extname(file)).replace(/[-_]/g, ' ');
              let family = rawName;
              let weightLabel = 'Regular';
              let weightValue = 400;

              const weightMatch = rawName.match(/\b(Thin|Hairline|ExtraLight|Light|Regular|Medium|SemiBold|Bold|ExtraBold|Black)\b/i);
              if (weightMatch) {
                family = rawName.replace(weightMatch[0], '').trim();
                const w = weightMatch[1].toLowerCase();
                if (w === 'thin') { weightLabel = 'Thin'; weightValue = 100; }
                else if (w === 'extralight') { weightLabel = 'Extra Light'; weightValue = 200; }
                else if (w === 'light') { weightLabel = 'Light'; weightValue = 300; }
                else if (w === 'medium') { weightLabel = 'Medium'; weightValue = 500; }
                else if (w === 'semibold') { weightLabel = 'Semi Bold'; weightValue = 600; }
                else if (w === 'bold') { weightLabel = 'Bold'; weightValue = 700; }
                else if (w === 'extrabold') { weightLabel = 'Extra Bold'; weightValue = 800; }
                else if (w === 'black') { weightLabel = 'Black'; weightValue = 900; }
              }
              if (!family) family = rawName;
              if (!fontFamilies.has(family)) fontFamilies.set(family, []);
              const weights = fontFamilies.get(family)!;
              if (!weights.find(w => w.value === weightValue)) {
                weights.push({ label: weightLabel, value: weightValue, fullName: rawName });
              }
            }
          }
        } catch (_) {}
